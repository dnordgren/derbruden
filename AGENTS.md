...
- Shared page CSS lives in `static/css/site.css`. Pages link it and keep
  only page-specific rules in a small inline `<style>`. The drafts and
  power-rankings pages are fully regenerated by their scripts, so head
  changes (fonts, stylesheet links, speculation rules) must be made in
  `scripts/generate-drafts.js` / `generate-power-rankings.js` templates,
  not just the generated HTML.
